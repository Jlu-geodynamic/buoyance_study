...
#include </fs2/home/liuzhonglan/wy/lib_extra/grainsize20230317/grain/include/visco_plastic_grainsize.h>
			...
			const bool k_modify_active = k_modify_order && plastic_strain > 0.1 ? 1:0;
			  ...
			  else if (k_modify_active)
			  {
				  ...
				  if (this->get_parameters().formulation_temperature_equation ==
                      Parameters<dim>::Formulation::TemperatureEquation::reference_density_profile &&
                      this->get_adiabatic_conditions().is_initialized())
                    out.thermal_conductivities[i] = active_factor * thermal_diffusivity * out.specific_heat[i] *
                                                    this->get_adiabatic_conditions().density(in.position[i]);
                  else
                    out.thermal_conductivities[i] = active_factor * thermal_diffusivity * out.specific_heat[i] * out.densities[i];
			  }
